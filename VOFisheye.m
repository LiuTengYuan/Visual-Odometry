function VOFisheye(time,ratioVelocity,windowSize,normalization)

%Output #5: VO script using the fisheye camera simulator + example of VO output on a simple trajectory

clearvars -except time ratioVelocity windowSize normalization Ntimes; close all; clc;
tic;
...
%% Change Velocity (same trajectory)
if ratioVelocity > 1
    if mod(Npose,ratioVelocity)==0
        VS = ratioVelocity*VS; %final velocity = ratio_V * initial velocity
        x0_vNEW = zeros(1,Npose/ratioVelocity); y0_vNEW = zeros(1,Npose/ratioVelocity); z0_vNEW = zeros(1,Npose/ratioVelocity);
        a_vNEW = zeros(1,Npose/ratioVelocity); b_vNEW = zeros(1,Npose/ratioVelocity); c_vNEW = zeros(1,Npose/ratioVelocity);
        j = 0;
        for i = 1:ratioVelocity:Npose
            j=  j+1;
            x0_vNEW(j) = x0_v(i);
            y0_vNEW(j) = y0_v(i);
            z0_vNEW(j) = z0_v(i);
        ...
        x0_v = x0_vNEW; y0_v = y0_vNEW; z0_v = z0_vNEW;
        a_v = a_vNEW; b_v = b_vNEW; c_v = c_vNEW;
        clear x0_vNEW y0_vNEW z0_vNEW a_vNEW b_vNEW c_vNEW
        Npose = Npose/ratioVelocity;
        ViewId = (1:Npose)';
        ...
        error('Wrong Velocity(%d)! Npose(%d) could not be divisible by ratio velocity(%d)!'...
            ,VS*ratioVelocity,Npose,ratioVelocity)
    end
end
...
for ind_pose = 1:Npose
    ...
    %     uniformfisheyePoints = selectUniform(SURFfisheyePoints,numPoints,...
    %         [Cfisheye(ind_pose,18) Cfisheye(ind_pose,17)]);
    ...
    switch ind_pose
        ...
        case num2cell(windowSize+1:Npose)
            indexPairs = DetectandMatchFeatures(preFeatures,currFeatures);
            [~, ~, inlierIdx] = helperEstimateRelativePose(...
                prevPoints(indexPairs(:,1),:), currPoints(indexPairs(:,2),:), cameraParams);
            indexPairs = indexPairs(inlierIdx, :);
            ...
            vSet = addView(vSet, viewId, 'Points', currPoints, 'Orientation', orient, ...
                'Location', loc);
            vSet = addConnection(vSet, viewId-1, viewId, 'Matches', indexPairs);
            if logical(windowSize) && mod(viewId,floor(windowSize/2)) == 0
    ...
    vSet_RT = addView(vSet_RT, viewId, 'Points', currPoints,'Orientation',...
        vSet.Views.Orientation{viewId}, 'Location', vSet.Views.Location{viewId});
    if viewId ~= 1
        vSet_RT = addConnection(vSet_RT, viewId-1, viewId, 'Matches', indexPairs);
    end
    ...
    clc;
    if exist('time','var')
        fprintf("\nVOFisheyeVR%dWZ%d%s_%03d processing...",ratioVelocity,windowSize,normalization,time);
    end
    fprintf("\nTotal Complete - %.2f %%\n",(ind_pose/Npose)*100);
end; clear ind_pose
t = toc;
...
if exist('time','var')
    filename = ['./results/test_fisheyeVR',num2str(ratioVelocity,'%d'),'WZ',...
        num2str(windowSize,'%d'),normalization,'_',num2str(time,'%03d'),'.mat'];
    save(filename,'vSet','vSet_RT','groundTruthPoses','t')
else
    filename = './results/test_fisheye.mat';
    save(filename)
end
