...
filename = './results/9_5/Performance/result_all/test_fisheye_2D2DNEW2.mat';
load(filename)

% h1 = figure;
% h2 = figure;
for ind_pose = Npose:Npose
    viewId = ind_pose;
    %% Draw World Scenario
%     % Fisheye example
%     %             figure(h1)
...
%     actual = plot3(x0_v(1:ind_pose),y0_v(1:ind_pose),z0_v(1:ind_pose),'b.-');
%     % Draw estimated camera positions
%     locations = cat(1,vSet.Views.Location{:});
%     estimated = plot3(locations(:,1),locations(:,2),locations(:,3),'g.-');
%     
%     % Draw World Points
%     Points = DrawPoints(XYZfisheye{ind_pose},[0 0 0]);
%     
%     % Draw image points in world frame
%     I2C = ImageToCamera(C(ind_pose,:));
%     C2W = CameraToWorld(C(ind_pose,:));
%     points_world = C2W*I2C*UV_fisheye{ind_pose}(:,ind_keep{ind_pose});
%     DrawPoints(points_world,[1 0 0]);
%     
%     % Draw corresponding world points
%     plot3(XYZfisheyeP{ind_pose}(1,ind_keep{ind_pose}),XYZfisheyeP{ind_pose}(2,ind_keep{ind_pose})...
%         ,XYZfisheyeP{ind_pose}(3,ind_keep{ind_pose}),...
...
%     
%     % Draw Fisheye Lines
%     DrawFisheyeProjectionLines([Cfisheye(ind_pose,1),Cfisheye(ind_pose,2),Cfisheye(ind_pose,3)]...
%         ,points_world,XYZfisheyeP{ind_pose}(:,ind_keep{ind_pose}))
%     
%     % Limit world
%     world_span = max(abs( [XYZfisheye{ind_pose}(1,:)-Cfisheye(ind_pose,1),XYZfisheye{ind_pose}(2,:)-...
%         Cfisheye(ind_pose,2),XYZfisheye{ind_pose}(3,:)-Cfisheye(ind_pose,3)]));
%     axis([Cfisheye(ind_pose,1)-world_span,Cfisheye(ind_pose,1)+world_span,...
%         Cfisheye(ind_pose,2)-world_span,Cfisheye(ind_pose,2)+world_span,...
%         Cfisheye(ind_pose,3)-world_span,Cfisheye(ind_pose,3)+world_span])
%     
%     view([45,45]);
...
%     title('Camera Trajectory');
    ...
    %% Draw image plane
    subplot(2,2,2);
    DrawImagePoints(UV_fisheye{ind_pose}(:,ind_keep{ind_pose}),Cfisheye(ind_pose,17),Cfisheye(ind_pose,18));
    title('Image with Perspective Camera Model');
    drawnow
    
    %% Save into a .fig file
%     saveas(gcf,['./results/image_fisheye_' num2str(ind_pose,'%03d') '.fig']);
...
axis tight
% saveas(gcf,['./results/image_fisheye_final.fig']);
toc;
return

%% Create animated gif from a list of .fig files
% filename = './results/trajectory_fisheye';
% for ind_pose = 1:Npose
%     % open .fig file
%     openfig(['./results/image_fisheye_' num2str(ind_pose,'%03d') '.fig']);
...
%     % close figure
%     close(gcf);
%     delete(['./results/image_fisheye_' num2str(ind_pose,'%03d') '.fig']);
%     % save to movie
%     if ind_pose == 1
...
filename = './results/cameraview_fisheye';
