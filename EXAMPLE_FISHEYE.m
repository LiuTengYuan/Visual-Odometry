...
Cfisheye=[x0,y0,z0,...%x0,y0,z0
    a*pi/180,b*pi/180,c*pi/180,...%a,b,c
    875.08,-3.04e-4,-4.82e-08,1.71e-11,...%a0,a2,a3,a4     500.08,-1.02e-3,-3.82e-06,1.71e-10,...
    640,360,...% cx,cy
    1,0,0,1,...% s1,s2,s3,s4
    1280,720];% Nu,Nv
Cfisheye=[x0,y0,z0,...%x0,y0,z0
...
% Limit world
world_span = max(abs( [XYZ(1,:)-Cfisheye(1),XYZ(2,:)-Cfisheye(2),...
    XYZ(3,:)-Cfisheye(3)]));
...
intrinsics = fisheyeIntrinsics([Cfisheye(7), Cfisheye(8), Cfisheye(9), Cfisheye(10)],...
    [Cfisheye(18), Cfisheye(17)], [Cfisheye(11), Cfisheye(12)],...
    [Cfisheye(13) Cfisheye(14);Cfisheye(15) Cfisheye(16)]);
%??????????????????????????????????????????????????????????????????????????
scaleFactor = [C(7)*C(10) C(7)*C(11)]/(min([C(13) C(12)])/2);
%??????????????????????????????????????????????????????????????????????????
[UV_undistorted, camIntrinsics, reprojectionErrors] = ...
    undistortFisheyePoints(UV_fisheye(1:2,ind_keep)', intrinsics, scaleFactor);
UV_undistorted = UV_undistorted';
UV_undistorted(3,:) = 1;
...
hold on
DrawImagePoints(UV_fisheye(:,ind_keep),Cfisheye(17),Cfisheye(18),[1 0 0]);
DrawImagePoints(UV_undistorted(:,ind_keep),Cfisheye(17),Cfisheye(18),[0 0 1],'o');
if ~isempty(UV_fisheye(:,ind_keep)) && ~isempty(UV_undistorted(:,ind_keep))
    legend('Perspective','Fisheye','Undistorted')
end
title('Image with Different Camera Model');
