...
% Setup axes.
figure('position', [700, 200, 750, 700])
subplot(2,1,1)
...
%check difference between estimated point from VO and actual point from
%groundTruthPose
Diff_EstimatedActual = NaN(numPoints,1);
Diff_EstimatedActual(viewId) = 0;
trajectorydiff = subplot(2,1,2); plot(Diff_EstimatedActual,'r-');
xlabel('views'); ylabel('error (cm)'); title('Error for Estimated Trajectory')
...
posesEstimated = poses(vSet);
Diff_EstimatedActual(viewId) = norm(posesEstimated.Location{viewId}-...
    groundTruthPoses.Location{viewId});
subplot(trajectorydiff); plot(Diff_EstimatedActual,'r-');
xlabel('views'); ylabel('error (cm)'); title('Error for Estimated Trajectory')

helperUpdateCameraPlots(viewId, camEstimated, camActual, posesEstimated, ...
    groundTruthPoses);
helperUpdateCameraTrajectories(viewId, trajectoryEstimated, trajectoryActual,...
    posesEstimated, groundTruthPoses);
...
for viewId = 3:15
    ...
    helperUpdateCameraTrajectories(viewId, trajectoryEstimated, ...
        trajectoryActual, posesEstimated, groundTruthPoses);
